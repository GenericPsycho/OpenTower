function baddiecollisionbox_update(player)
{
	...
	if player.cutscene == 0 && player.state != states.firemouth
	{
		with player
		{
			...
			{
				if (state == states.mach3 && sprite_index != spr_mach3hit && (character == "P" || character == "V" || character == "E"))
				{
					if fightball == 0
			...
			if (instance_exists(other.baddieID) && state == states.handstandjump && !other.baddieID.invincible)
			{
				...
				else if (other.baddieID.thrown == 0 && (character == "P" || character == "N" || character == "E"))
				{
					movespeed = 0;
					image_index = 0;
