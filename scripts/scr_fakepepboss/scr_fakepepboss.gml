...
function scr_fakepep_update_sounds()
{
	...
	if ((state == states.Sjumpprep || state == states.Sjump) && sprite_index != spr_fakepeppino_idle && sprite_index != spr_ditto_idle)
	{
	...
	if ((sprite_index == spr_fakepeppino_throwhead || sprite_index == spr_ditto_throwhead) && floor(image_index) >= 6)
	{
	...
	if sprite_index == spr_fakepeppino_flailing || sprite_index == spr_ditto_flailing
	{
...
function scr_fakepepboss_arenaintro()
{
	if !skipintro
	{
		...
		with obj_player
		{
			...
			if other.sprite_index == spr_fakepeppino_intro1 || other.sprite_index == spr_ditto_intro1
			{
				if ispeppino
		...
		if floor(image_index) == image_number - 1
		{
			if (sprite_index == spr_fakepeppino_intro1 || sprite_index == spr_fakepeppino_intro2 || sprite_index == spr_ditto_intro1 || sprite_index == spr_ditto_intro2)
				image_index = image_number - 1;
			else if sprite_index == spr_fakepeppino_intro3 || sprite_index == spr_ditto_intro3
				image_index = image_number - 4;
		}
		if introbuffer > 0
			introbuffer--;
		else if sprite_index == spr_fakepeppino_intro1 || sprite_index == spr_ditto_intro1
		{
			sprite_index = obj_player1.character == "E" ? spr_ditto_intro2 : spr_fakepeppino_intro2;
			image_index = 0;
			introbuffer = 70;
			with obj_player
			{
		...
		else if sprite_index == spr_fakepeppino_intro2 || sprite_index == spr_ditto_intro2
		{
			sprite_index = obj_player1.character == "E" ? spr_ditto_intro3 : spr_fakepeppino_intro3;
		...
		{
			sprite_index = obj_player1.character == "E" ? spr_ditto_intro4 : spr_fakepeppino_intro4;
			image_index = 0;
			introbuffer = 0;
		}
		else if ((sprite_index == spr_fakepeppino_intro4 || sprite_index == spr_ditto_intro4) && floor(image_index) == image_number - 1)
		{
			state = states.walk;
		...
		scr_boss_genericintro(obj_player1.character == "E" ? spr_ditto_idle : spr_fakepeppino_idle);
}
function scr_fakepepboss_walk()
{
	if (phase == 2 && (flickertime > 0 || !grounded))
	{
		...
		cooldown = 0;
		sprite_index = obj_player1.character == "E" ? spr_ditto_flicker : spr_fakepeppino_flicker;
		exit;
	}
	if formed
	{
		...
		else if grounded && vsp > 0
		{
			...
			switch attack.attack
			{
				...
				case fakepep_attacks.bodyslamclone:
					if ix != 0
						image_xscale = ix;
					sprite_index = obj_player1.character == "E" ? spr_ditto_bodyslamstart : spr_fakepeppino_bodyslamstart;
					image_index = 0;
					state = states.jump;
					cooldown = attack.cooldown;
					break;
...
function scr_fakepepboss_deformed()
{
	...
	vsp = 0;
	var t = targetplayer;
	if ((sprite_index == spr_fakepeppino_deform || sprite_index == spr_ditto_deform) && floor(image_index) == image_number - 1)
		image_index = image_number - 1;
...
function scr_fakepepboss_grabdash()
{
	if grabstart > 0
	{
		...
		if grabstart <= 0
		{
			sprite_index = obj_player1.character == "E" ? spr_ditto_grabdash : spr_fakepeppino_grabdash;
			image_index = 0;
		}
	}
	else
	{
		...
		with (instance_place(x, y, obj_player))
		{
			if !hurted && state != states.grabthrow && state != states.tackle && state != states.actor && state != states.supergrab
			{
				...
				other.playerID = id;
				other.sprite_index = character == "E" ? spr_ditto_grabattack :  spr_fakepeppino_grabattack;
				other.image_index = 0;
				invtime = 0;
...
function scr_fakepepboss_jump()
{
	if obj_player1.character == "E"
	{
		if sprite_index == spr_ditto_bodyslamstart
		{
			hsp = 0;
			vsp = 0;
			if x != targetplayer.x
			...
			if floor(image_index) == image_number - 1
			{
				sprite_index = spr_ditto_bodyslamjump;
				attackspeed = 8;
				vsp = -18;
				create_particle(x, y, particle.highjumpcloud2);
			}
		...
		{
			if (sprite_index == spr_ditto_bodyslamjump && floor(image_index) == image_number - 1)
				image_index = image_number - 3;
			hsp = image_xscale * attackspeed;
			if vsp > 0
			{
				state = states.freefall;
				sprite_index = spr_ditto_bodyslamfall;
				image_index = 0;
...
{
	if ((sprite_index == spr_fakepeppino_bodyslamfall || sprite_index == spr_ditto_bodyslamfall) && floor(image_index) == image_number - 1)
		image_index = image_number - 4;
	if object_index == obj_fakepepclone && bodyslam && vsp > 2
		vsp += 0.5;
	if (sprite_index != spr_fakepeppino_bodyslamland && sprite_index != spr_ditto_bodyslamland) && !bodyslam
	{
	...
	if grounded && vsp > 0 && sprite_index != spr_fakepeppino_bodyslamland && sprite_index != spr_ditto_bodyslamland
	{
		fmod_event_one_shot_3d("event:/sfx/pep/groundpound", x, y);
		create_particle(x, y, particle.landcloud);
		sprite_index = obj_player1.character == "E" ? spr_ditto_bodyslamland : spr_fakepeppino_bodyslamland;
		image_index = 0;
	...
	if bodyslamlandbuffer > 0
		bodyslamlandbuffer--;
	if (sprite_index == spr_fakepeppino_bodyslamland || sprite_index == spr_ditto_bodyslamland) && bodyslamlandbuffer <= 0
		state = states.walk;
}
function scr_fakepepboss_mach2()
{
	...
	{
		case 0:
			sprite_index = obj_player1.character == "E" ? spr_ditto_mach : spr_fakepeppino_mach;
			hsp = image_xscale * attackspeed;
			if (attackspeed > 3 && place_meeting(x + sign(hsp), y, obj_solid))
			{
				machdir = 1;
				sprite_index = obj_player1.character == "E" ? spr_ditto_machwall : spr_fakepeppino_machwall;
			}
			break;
		...
		case 1:
			...
			if (place_meeting(x, y - 1, obj_solid))
			{
				machdir = 2;
				image_xscale *= -1;
				sprite_index = obj_player1.character == "E" ? spr_ditto_machceiling : spr_fakepeppino_machceiling;
			}
			break;
		...
		case 2:
			...
			if !bodyslam
			{
				...
				{
					sprite_index = obj_player1.character == "E" ? spr_ditto_machwalldown : spr_fakepeppino_machwalldown;
					machdir = 3;
				}
			}
			...
			{
				state = states.freefall;
				sprite_index = obj_player1.character == "E" ? spr_ditto_bodyslamfall : spr_fakepeppino_bodyslamfall;
				bodyslamlandbuffer = 0;
				hsp = 0;
			}
			break;
...
function scr_fakepepboss_Sjumpprep()
{
	...
	if floor(image_index) == image_number - 1
	{
		...
		if hitvsp < 0
			...
			sprite_index = obj_player1.character == "E" ? spr_ditto_superjumpdown : spr_fakepeppino_superjumpdown;
	}
}
	...
	else if (place_meeting(x, y + (sign(vsp) * yy), obj_solid))
	{
		...
		if (superjumpbounce > 0 || superjumpdir == 0)
		{
			...
			if hitvsp > 0
			{
				sprite_index = obj_player1.character == "E" ? spr_ditto_superjumpstart : spr_fakepeppino_superjumpstart;
				repeat 10
					create_debris(x + irandom_range(-25, 25), y + 43, obj_player1.character == "E" ? spr_dittodebris : spr_fakepepdebris);
			...
			{
				sprite_index = obj_player1.character == "E" ? spr_ditto_superjumpdownstart : spr_fakepeppino_superjumpdownstart;
				repeat 10
					create_debris(x + irandom_range(-25, 25), y, obj_player1.character == "E" ? spr_dittodebris : spr_fakepepdebris);
...
function scr_fakepepboss_throwing()
{
	...
	else if sprite_index == spr_fakepeppino_flailing || sprite_index == spr_ditto_flailing
	{
		...
		with headID
		{
			...
			{
				instance_destroy();
				other.sprite_index = obj_player1.character == "E" ? spr_ditto_gethead : spr_fakepeppino_gethead;
				other.image_index = 0;
				other.hsp = 0;
			}
		}
	}
	else if ((sprite_index == spr_fakepeppino_gethead || sprite_index == spr_ditto_gethead) && floor(image_index) == image_number - 1)
		state = states.walk;
}
function scr_fakepepboss_backbreaker()
{
	...
	else if (sprite_index == spr_ditto_jump && floor(image_index) == image_number - 1)
	...
	else if vsp > 0 && sprite_index == spr_ditto_fall && !taunted
	{
		taunted = true;
		sprite_index = choose(spr_ditto_supertaunt1, spr_ditto_supertaunt2, spr_ditto_supertaunt3);
		image_index = 0;
		with obj_camera
		{
			shake_mag = 3;
	...
	else if (sprite_index == spr_ditto_supertaunt1 || sprite_index == spr_ditto_supertaunt2 || sprite_index == spr_ditto_supertaunt3)
	{
		vsp = 0;
		if floor(image_index) == image_number - 1
			sprite_index = spr_ditto_fall;
