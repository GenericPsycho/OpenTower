...
{
	if sprite_index == spr_player_ratmountgroundpound || sprite_index == spr_playerK_ratmountgroundpound
	{
	...
	if gustavokicktimer == 0
	{
		if sprite_index == spr_lonegustavo_jumpstart || sprite_index == spr_lonepika_jumpstart
		{
			vsp = -11;
	...
	if sprite_index == spr_lonegustavo_jumpstart && sprite_index == spr_lonepika_jumpstart && gustavokicktimer > 0
	{
		vsp = 0;
		hsp = 0;
	}
	if move != 0
	{
		if sprite_index == spr_lonegustavo_groundpoundland || sprite_index == spr_lonepika_groundpoundland
	...
	if ((sprite_index == spr_lonegustavo_groundpoundstart || sprite_index == spr_lonepika_groundpoundstart) && floor(image_index) == image_number - 1)
	{
		image_index = 0;
	...
	if (sprite_index == spr_lonegustavo_groundpound || sprite_index == spr_lonegustavo_groundpoundstart || sprite_index == spr_lonepika_groundpound || sprite_index == spr_lonepika_groundpoundstart)
	{
		vsp++;
		if (grounded && vsp > 0 && !place_meeting(x, y + vsp + 2, obj_grindrail) && !place_meeting(x, y + 10, obj_grindrail) && !place_meeting(x, y + vsp, obj_destructibles) && !place_meeting(x, y + vsp + 2, obj_destructibles) && !place_meeting(x, y + 10, obj_destructibles))
		{
			...
			else
			{
				...
				if character == "E"
					sprite_index = spr_lonepika_groundpoundland;
				image_index = 0;
				jumpAnim = true;
				jumpstop = false;
				with obj_camera
	...
	if ((sprite_index == spr_lonegustavo_groundpoundland || sprite_index == spr_lonepika_groundpoundland) && floor(image_index) == image_number - 1)
	{
		if (sign(hsp) != 0)
			xscale = sign(hsp);
	...
	if ((sprite_index == spr_lonegustavo_kick || sprite_index == spr_lonegustavo_jumpstart || sprite_index == spr_lonepika_kick || sprite_index == spr_lonepika_jumpstart) && floor(image_index) == image_number - 1)
	{
		if (sign(hsp) != 0)
			xscale = sign(hsp);
		if sprite_index == spr_lonegustavo_kick || sprite_index == spr_lonepika_kick
		{
			sprite_index = spr_player_ratmountgroundpoundfall;
			if character == "E"
				sprite_index = spr_playerK_ratmountgroundpoundfall;
